...
from symbols.model_factory import ResNet, UnsuperVggTiny, InverseGain, ViT
...
class UnSuperPoint(ModelTemplate):
    ...
    def get_position(self, p_map, cell, downsample, flag=None, mat=None):
        ...
        if flag == 'A':
            ...
            # plt.figure()
            # plt.imshow(r[0,:,:].cpu().detach().numpy())
            # plt.show()
            return r
        else:
            return res
...
if __name__ == '__main__':
    ...
    cfg = None
    with open('../Unsuper/configs/Unsuper.yaml', 'r') as f:
        cfg = yaml.load(f, Loader=yaml.FullLoader)
    f.close()

    model = get_sym(model_config=cfg['MODEL'], image_shape=cfg['MIT_data']['IMAGE_SHAPE'], is_training=True)
    model.eval()
