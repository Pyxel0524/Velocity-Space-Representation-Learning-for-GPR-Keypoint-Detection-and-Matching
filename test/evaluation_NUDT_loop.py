...
def parse_config():
    ...
    parser.add_argument( "--kps", type=str, default="SIFT", required=False, help="VIFT,MigrationNet,ParNet,SIFT,ORB")
    parser.add_argument( "--des", type=str, default="SIFT", required=False, help="Method for Extract Descriptor")
    ...
    for kp in kps:
        for de in des:
            for nu in kp_num:
                ...
                config.keypoint_num = nu
                print('-----------------%d th: kps: %s; des: %s; kp_num: %d------------------'%(kk , kp,de,nu))
                ...
                if args.kps == 'MigrationNet':
                    ...
                    ckpt_dir = '../output/ckpt/VIFT++_NUDT.pth'
                    ...
                    ckpt_dir = '../output/ckpt/checkpoint_NUDT.pth' # ablation: VIFT_MIT_withoutusp.pth, VIFT_MIT_withoutRegularDecorr.pth, silk (without decorr)
                    model = get_sym(model_config=cfg['MODEL'], image_shape=cfg['MIT_data']['IMAGE_SHAPE'])
                    model.load_state_dict(torch.load(ckpt_dir, map_location=device)['model_state'])
                    # model.load_params_from_file(filename=ckpt_dir, logger=logger, to_cpu=dist_test)
                    model.cuda()
                ...
                if args.des == 'Silk':
                    ...
                    ckpt_dir = '../output/ckpt/VIFT++_NUDT.pth'  # VIFT_MIT_withoutRegularDecorr.pth, MIT_checkpoint_epoch_46.pth, VIFT_MIT_withoutusp.pth
