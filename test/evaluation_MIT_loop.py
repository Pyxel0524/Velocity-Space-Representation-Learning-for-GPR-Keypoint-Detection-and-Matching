        ...
        for de in des:
            for nu in kp_num:
                ...
                if args.kps == 'MigrationNet':
                    ...
                    ckpt_dir = '../output/ckpt/VIFT++_MIT.pth'#VIFT_MIT_withoutRegularDecorr.pth/MIT_checkpoint_epoch_46.pth
                    ...
                    ckpt_dir = '../output/ckpt/VIFT++_MIT.pth' # ablation: VIFT_MIT_withoutusp.pth, VIFT_MIT_withoutRegularDecorr.pth, silk (without decorr)
                    model = get_sym(model_config=cfg['MODEL'], image_shape=cfg['MIT_data']['IMAGE_SHAPE'])
                    model.load_state_dict(torch.load(ckpt_dir, map_location=device)['model_state'])
                    # model.load_params_from_file(filename=ckpt_dir, logger=logger, to_cpu=dist_test)
                    model.cuda()
                ...
                if args.des == 'Silk':
                    ...
                    ckpt_dir = '../output/ckpt/VIFT++_MIT.pth'  # VIFT_MIT_withoutRegularDecorr.pth, MIT_checkpoint_epoch_46.pth, VIFT_MIT_withoutusp.pth
                ...
                with torch.no_grad():
                    ...
                    for i in range(start_ind, end_ind, config.stride):# start_ind, end_ind, config.stride
                        ...
                        points1, descriptors1 = extract_kp_des(config, model, descmodel, radar_echo1, radar_point_utm1,
                                                               channel_num = 11, keypoint_num=config.keypoint_num, kp_method=args.kps, desc_method=args.des, nms=False, depth_scale = args.depth_scale ,downsample=cfg['MODEL']['downsample'])
