...
def train_one_epoch(model, optimizer, train_loader, lr_scheduler, accumulated_iter, optim_cfg,
                    rank, tbar, total_it_each_epoch, dataloader_iter, device, curr_epoch, tb_log=None, leave_pbar=False,
                    velocity=5):
    ...
    for cur_it in range(total_it_each_epoch):
        try:
            gpr_echoes = next(dataloader_iter)
        except StopIteration:
            dataloader_iter = iter(train_loader)
            gpr_echoes = next(dataloader_iter)
            print('new iters')

        # gpr_echoes = gpr_echoes.cuda()
        gpr_echoes = gpr_echoes.to(device)
        try:
        ...
        B, C, H, W = gpr_echoes.shape
        gpr_echoes = gpr_echoes.view(-1, 1, H, W)
        s, p, d = model.forward(gpr_echoes)

        if cur_it % 30 == 0:
            save_path = './output/valid/' + str(curr_epoch) + '_' + str(cur_it) + '.png'
            print('------------save score map----------')
            save_image(s, save_path, nrow=5, padding=5)

        # import matplotlib.pyplot as plt
        # plt.imshow(s[0,0,:,:].detach().cpu().numpy()); plt.colorbar()
        # plt.show()
        loss, loss_item = model.loss(gpr_echoes, s, p, d, velocity)
...
def train_model(model, optimizer, train_loader, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, device, ckpt_save_dir, train_sampler=None,
                lr_warmup_scheduler=None, ckpt_save_interval=1, max_ckpt_save_num=50, scale=5):
