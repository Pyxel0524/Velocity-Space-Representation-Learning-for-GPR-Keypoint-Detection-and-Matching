...
def train_one_epoch(model, optimizer, train_loader, lr_scheduler, accumulated_iter, optim_cfg,
    ...
    for cur_it in range(total_it_each_epoch):
        try:
            echo1, echo2 = next(dataloader_iter)
        ...
        if cur_it == 0 and rank == 0:
            img0_grid = torchvision.utils.make_grid(echo1/255.0)
            img1_grid = torchvision.utils.make_grid(echo2/255.0)

            tb_log.add_image('original_images', img0_grid)
            tb_log.add_image('homography', img1_grid)

        echo1 = echo1.cuda().to(torch.float32).unsqueeze(1)
        echo2 = echo2.cuda().to(torch.float32).unsqueeze(1)
        ...
        s1, p1, d1 = model.forward(echo1)
        s2, p2, d2 = model.forward(echo2)
        # import matplotlib.pyplot as plt
        # plt.imshow(s1[0,0,:,:].detach().cpu().numpy(),cmap = 'jet');
        # plt.figure(); plt.imshow(s2[0,0,:,:].detach().cpu().numpy(),cmap = 'jet');
        # plt.figure(); plt.imshow(echo1[0,0,:,:].detach().cpu().numpy());plt.plot()
        # plt.figure(); plt.imshow(echo2[0,0,:,:].detach().cpu().numpy()); plt.plot()
        # plt.show()
        loss, loss_item = model.loss(echo1, s1, p1, d1, echo2, s2, p2, d2)
